...
# Variables
rofi_theme="$HOME/.config/rofi/themes/rofi-wall-sel.rasi"
...
apply_image_wallpaper() {
  ...
  if command -v wal >/dev/null 2>&1; then
    ...
    notify-send "pywal no encontrado" "Instala 'pywal' para aplicar la paleta automáticamente."
  fi

  # Opcional: aplicar colores en terminales al vuelo (si los usas)
  if command -v kitty >/dev/null 2>&1; then
    kitty @ set-colors --all ~/.cache/wal/colors-kitty.conf >/dev/null 2>&1 || true
  fi
